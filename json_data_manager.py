...
class JSONDataManager:
    ...
    def calculate_sizes(self, json_string: str, compressed_data: bytes = None) -> Dict[str, float]:
        ...
        return {
            ...
            'compressed_size_kb': round(compressed_size_kb, 2) if compressed_size_kb else None,
            'compression_ratio': round(compression_ratio, 2) if compression_ratio else None
        }
    
    def prepare_servicenow_data(self, dataframes: Dict[str, pl.DataFrame]) -> Dict[str, Any]:
            ...
            with get_db_connection() as conn:
                ...
                try:
                    ...
                    if start_date and end_date:
                        ...
                        """
                        cursor.execute(delete_query, start_date, end_date, extraction_type)
                        ...
                        """
                        cursor.execute(delete_query, data_extraction, extraction_type)
                    ...
                    api_requests = extraction_metrics.get('total_requests', 0) if extraction_metrics else 0
                    ...
                    self._print_save_summary(size_metrics, record_count, duration)
    ...
    def _update_metrics(self, size_metrics: Dict, record_count: int):
        ...
        if size_metrics['compressed_size_kb']:
            self.json_metrics['total_compressed_size_kb'] += size_metrics['compressed_size_kb']
            
            # Calcula média da razão de compressão
            if self.json_metrics['avg_compression_ratio'] == 0:
                self.json_metrics['avg_compression_ratio'] = size_metrics['compression_ratio']
            else:
                self.json_metrics['avg_compression_ratio'] = (
                    self.json_metrics['avg_compression_ratio'] + size_metrics['compression_ratio']
                ) / 2
    
    def _print_save_summary(self, size_metrics: Dict, record_count: int, duration: float):
        ...
        if size_metrics['compressed_size_kb']:
            print(f"   ├── Tamanho comprimido: {size_metrics['compressed_size_kb']:.2f} KB")
            print(f"   ├── Economia: {size_metrics['compression_ratio']:.1f}%")
        
        print(f"   └── Tempo de processamento: {duration:.2f}s")
    
    def load_json_data_from_db(self, data_extraction: str, extraction_type: str = 'daily') -> Optional[Dict]:
    ...
    def print_json_metrics(self):
        ...
        avg_compressed_size = metrics['total_compressed_size_kb'] / metrics['total_saves'] if metrics['total_compressed_size_kb'] > 0 else 0
        ...
        print(f"   ├── Tamanho total JSON: {metrics['total_json_size_kb']:.2f} KB")
        print(f"   ├── Tamanho médio por save: {avg_json_size:.2f} KB")
        
        if avg_compressed_size > 0:
            print(f"   ├── Tamanho total comprimido: {metrics['total_compressed_size_kb']:.2f} KB")
