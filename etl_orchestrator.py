...
class ServiceNowETLOrchestrator:
    ...
    def sync_reference_data(self, force_full_sync: bool = False) -> bool:
        ...
        try:
            ...
            df_users = self.user_extractor.extract_data(force_full_sync=force_full_sync)
            ...
            if not df_missing_users.is_empty():
                ...
                print(f"✅ {len(df_missing_users)} usuários em falta sincronizados")
            
            # 3. Sincroniza empresas
            print("\n🏢 Sincronizando empresas...")
            df_companies = self.company_extractor.extract_data(force_full_sync=force_full_sync)
            ...
            df_missing_companies = self.company_extractor.sync_missing_companies()
            
            if not df_missing_companies.is_empty():
                save_to_database(df_missing_companies, "sys_company")
                print(f"✅ {len(df_missing_companies)} empresas em falta sincronizadas")
    ...
    def full_etl_workflow(
        ...
        try:
            ...
            ref_success = self.sync_reference_data(force_full_sync=force_full_reference_sync)
            
            if not ref_success:
                print("⚠️ Aviso: Problemas na sincronização de referência, mas continuando...")
    ...
    def sync_specific_companies(self, company_type: str = "customer") -> bool:
        ...
        try:
            df_companies = self.company_extractor.get_companies_by_type(company_type)
            
            if not df_companies.is_empty():
                save_to_database(df_companies, "sys_company")
                print(f"✅ {len(df_companies)} empresas {company_type} sincronizadas")
                return True
            else:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  incidents YYYY-MM-DD YYYY-MM-DD - Extrair incidentes do período")
    ...
    if command == "ref-sync":
        ...
        if len(sys.argv) < 4:
            print("Uso: python etl_orchestrator.py incidents YYYY-MM-DD YYYY-MM-DD")
            sys.exit(1)
        ...
        if len(sys.argv) < 4:
            print("Uso: python etl_orchestrator.py full-etl YYYY-MM-DD YYYY-MM-DD [--full-ref]")
            sys.exit(1)
