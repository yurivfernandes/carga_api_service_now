...
class UserExtractor(BaseServiceNowExtractor):
    ...
    def _extract_all_users(self) -> pl.DataFrame:
        ...
        inactive_users = self.paginated_request(self.api_endpoint, params_inactive)
        print(f"✅ {len(inactive_users)} usuários inativos modificados recentemente")
    ...
    def _extract_incremental_users(self) -> pl.DataFrame:
        ...
        print(f"✅ {len(modified_users)} usuários modificados desde {last_sync_date}")
        ...
        if modified_users:
            ...
            print(f"🔄 {len(users_to_update)} usuários com mudanças reais detectadas")
    ...
    def _get_user_fields(self) -> str:
        ...
        fields = [
            # Campos básicos
            "sys_id", "user_name", "name", "first_name", "last_name", "middle_name",
            
            # Contato
            "email", "phone", "mobile_phone",
            
            # Organizacional
            "company", "department", "location", "manager", "title",
            
            # Status
            "active", "locked_out", "web_service_access_only",
            
            # Login
            "last_login", "last_login_time", "failed_attempts",
    ...
    def _process_and_hash_users(self, users: List[dict]) -> List[dict]:
        ...
        for user in users:
            ...
            hash_data = {k: v for k, v in processed_user.items() 
                        if not k.startswith('etl_') and k not in ['sys_created_on', 'sys_updated_on']}
            
            data_string = str(sorted(hash_data.items()))
            processed_user['etl_hash'] = hashlib.md5(data_string.encode()).hexdigest()
        ...
        try:
            with get_db_connection() as conn:
                ...
                for user in users:
                    ...
                    if result is None:
                        # Usuário novo
                        print(f"➕ Novo usuário: {user.get('user_name', user.get('sys_id'))}")
                        changed_users.append(user)
                    elif result[0] != user['etl_hash']:
                        # Usuário modificado
                        print(f"🔄 Usuário modificado: {user.get('user_name', user.get('sys_id'))}")
                        changed_users.append(user)
    ...
    def _get_last_sync_date(self) -> str:
        ...
        default_date = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def get_users_by_ids(self, user_ids: Set[str]) -> pl.DataFrame:
        ...
        for i in range(0, len(user_ids_list), batch_size):
            ...
            print(f"📥 Lote {i//batch_size + 1}: {len(batch_users)} usuários")
    ...
    def get_missing_user_ids_from_incidents(self) -> Set[str]:
        ...
        try:
            with get_db_connection() as conn:
                ...
                print(f"🔍 {len(missing_ids)} usuários referenciados mas não encontrados na tabela")
