...
class CompanyExtractor(BaseServiceNowExtractor):
    ...
    def __init__(self):
        ...
        self.api_endpoint = "core_company"  # Endpoint do ServiceNow para empresas
    ...
    def _extract_all_companies(self) -> pl.DataFrame:
        ...
        inactive_companies = self.paginated_request(self.api_endpoint, params_inactive)
        print(f"✅ {len(inactive_companies)} empresas inativas modificadas recentemente")
        ...
        if unique_companies:
            processed_companies = self._process_and_hash_companies(unique_companies)
    ...
    def _extract_incremental_companies(self) -> pl.DataFrame:
        ...
        print(f"✅ {len(modified_companies)} empresas modificadas desde {last_sync_date}")
        
        if modified_companies:
            processed_companies = self._process_and_hash_companies(modified_companies)
            
            # 3. Identifica quais realmente mudaram (usando hash)
            companies_to_update = self._filter_changed_companies(processed_companies)
            print(f"🔄 {len(companies_to_update)} empresas com mudanças reais detectadas")
        ...
        for company in companies:
            ...
            for field in bool_fields:
                if field in processed_company:
                    processed_company[field] = 1 if processed_company[field] in [True, 'true', '1'] else 0
            ...
            hash_data = {k: v for k, v in processed_company.items() 
                        if not k.startswith('etl_') and k not in ['sys_created_on', 'sys_updated_on']}
            
            data_string = str(sorted(hash_data.items()))
            processed_company['etl_hash'] = hashlib.md5(data_string.encode()).hexdigest()
        ...
        try:
            with get_db_connection() as conn:
                ...
                for company in companies:
                    ...
                    if result is None:
                        # Empresa nova
                        print(f"➕ Nova empresa: {company.get('name', company.get('sys_id'))}")
                        changed_companies.append(company)
                    elif result[0] != company['etl_hash']:
                        # Empresa modificada
                        print(f"🔄 Empresa modificada: {company.get('name', company.get('sys_id'))}")
                        changed_companies.append(company)
    ...
    def get_companies_by_ids(self, company_ids: Set[str]) -> pl.DataFrame:
        ...
        for i in range(0, len(company_ids_list), batch_size):
            ...
            print(f"📥 Lote {i//batch_size + 1}: {len(batch_companies)} empresas")
        
        print(f"✅ Total de {len(all_companies)} empresas específicas extraídas")
        
        if all_companies:
            processed_companies = self._process_and_hash_companies(all_companies)
            return pl.DataFrame(processed_companies)
        else:
            return pl.DataFrame()
    ...
    def get_missing_company_ids_from_incidents(self) -> Set[str]:
        ...
        try:
            with get_db_connection() as conn:
                ...
                print(f"🔍 {len(missing_ids)} empresas referenciadas mas não encontradas na tabela")
    ...
    def get_companies_by_type(self, company_type: str = "customer") -> pl.DataFrame:
        """
        ...
        print(f"✅ {len(companies)} empresas do tipo {company_type} encontradas")
