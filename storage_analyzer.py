...
class StorageAnalyzer:
        ...
        with get_db_connection() as conn:
            ...
            try:
                ...
                for row in results:
                    ...
                    json_sizes['total_compressed_size_kb'] += entry['compressed_size_kb']
                
                # Calcula média da compressão
                if json_sizes['entries']:
                    valid_ratios = [e['compression_ratio'] for e in json_sizes['entries'] if e['compression_ratio'] > 0]
                    if valid_ratios:
                        json_sizes['average_compression_ratio'] = sum(valid_ratios) / len(valid_ratios)
    ...
    def calculate_space_efficiency(self) -> Dict[str, Any]:
        ...
        normalized_total_records = sum(table['records'] for table in normalized_sizes.values())
        normalized_total_size_kb = sum(table['size_kb'] for table in normalized_sizes.values())
        ...
        efficiency = {
            'normalized': {
                ...
                'avg_bytes_per_record': round((normalized_total_size_kb * 1024) / normalized_total_records, 2) if normalized_total_records > 0 else 0,
                'tables': normalized_sizes
            },
            'json': {
                'total_records': json_sizes['total_records'],
                'json_size_kb': round(json_sizes['total_json_size_kb'], 2),
                'compressed_size_kb': round(json_sizes['total_compressed_size_kb'], 2),
                'avg_compression_ratio': round(json_sizes['average_compression_ratio'], 2),
                'avg_bytes_per_record_json': round((json_sizes['total_json_size_kb'] * 1024) / json_sizes['total_records'], 2) if json_sizes['total_records'] > 0 else 0,
                'avg_bytes_per_record_compressed': round((json_sizes['total_compressed_size_kb'] * 1024) / json_sizes['total_records'], 2) if json_sizes['total_records'] > 0 else 0,
                'entries': json_sizes['entries']
            }
        }
        
        # Calcula comparações se há dados suficientes
        if normalized_total_size_kb > 0 and json_sizes['total_compressed_size_kb'] > 0:
            space_saved_vs_normalized = ((normalized_total_size_kb - json_sizes['total_compressed_size_kb']) / normalized_total_size_kb) * 100
            efficiency['comparison'] = {
                'space_saved_vs_normalized_percent': round(space_saved_vs_normalized, 2),
                'json_vs_normalized_ratio': round(json_sizes['total_compressed_size_kb'] / normalized_total_size_kb, 3),
                'recommendation': self._get_recommendation(space_saved_vs_normalized)
            }
        else:
            efficiency['comparison'] = {
    ...
    def print_detailed_analysis(self):
        ...
        print(f"💾 Tamanho total: {normalized['total_size_kb']:,.2f} KB ({normalized['total_size_kb']/1024:.2f} MB)")
        print(f"⚖️ Média por registro: {normalized['avg_bytes_per_record']:.1f} bytes")
        
        print("\n📋 Detalhamento por tabela:")
        for table, data in normalized['tables'].items():
            print(f"   ├── {table:20} {data['records']:>8,} registros  {data['size_kb']:>8.1f} KB")
        ...
        if json_data['total_records'] > 0:
            ...
            print(f"💾 Tamanho JSON: {json_data['json_size_kb']:,.2f} KB ({json_data['json_size_kb']/1024:.2f} MB)")
            print(f"🗜️ Tamanho comprimido: {json_data['compressed_size_kb']:,.2f} KB ({json_data['compressed_size_kb']/1024:.2f} MB)")
            print(f"📊 Compressão média: {json_data['avg_compression_ratio']:.1f}%")
            print(f"⚖️ Média por registro (JSON): {json_data['avg_bytes_per_record_json']:.1f} bytes")
            print(f"⚖️ Média por registro (comprimido): {json_data['avg_bytes_per_record_compressed']:.1f} bytes")
            
            if json_data['entries']:
                print("\n📋 Extrações JSON realizadas:")
                for entry in json_data['entries'][-5:]:  # Últimas 5 extrações
                    compression_display = f"{entry['compression_ratio']:.1f}%" if entry['compression_ratio'] > 0 else "N/A"
                    print(f"   ├── {entry['date']} ({entry['type']}) - {entry['records']:,} reg. - {entry['compressed_size_kb']:.1f} KB - {compression_display}")
        else:
        ...
        if comparison.get('space_saved_vs_normalized_percent', 0) != 0:
            ...
            print(f"📏 Razão JSON/Normalizado: {ratio:.3f} ({ratio*100:.1f}%)")
            print(f"🎯 Recomendação: {comparison['recommendation']}")
            
            # Projeções
            if space_saved > 0:
                print("\n📊 PROJEÇÕES:")
                annual_savings_mb = (normalized['total_size_kb'] * space_saved / 100) / 1024 * 12  # Assumindo crescimento mensal
                print(f"   ├── Economia anual estimada: {annual_savings_mb:.1f} MB")
                print(f"   └── Em 5 anos: {annual_savings_mb * 5:.1f} MB ({annual_savings_mb * 5 / 1024:.2f} GB)")
        else:
    ...
    def export_analysis_to_csv(self, output_file: str = "storage_analysis.csv"):
