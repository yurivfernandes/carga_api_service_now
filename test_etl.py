...
def test_full_workflow():
    ...
    success = orchestrator.full_etl_workflow(start_date, end_date, force_full_reference_sync=False)
    print(f"Resultado: {'✅ Sucesso' if success else '❌ Falha'}")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python test_etl.py ref         - Testa sincronização de referências")
        print("  python test_etl.py incidents   - Testa extração de incidentes")
        print("  python test_etl.py full        - Testa fluxo completo")
        print("  python test_etl.py quick       - Testa sincronização rápida")
        print("  python test_etl.py companies   - Testa sincronização de empresas")
        return
