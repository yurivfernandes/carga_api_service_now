...
class ExecutionLogger:
    ...
    def update_db_metrics(self, db_manager: Any):
        ...
        if hasattr(db_manager, "get_db_metrics_data"):
            ...
            self.execution_data.update(
                {
                    "total_db_transactions": db_metrics.get(
                        "total_transactions", 0
                    ),
                    "total_records_processed": db_metrics.get(
                        "total_records", 0
                    ),
                    "db_time_seconds": round(
                        db_metrics.get("total_time", 0.0), 2
                    ),
                }
            )
        ...
        if hasattr(json_manager, "get_json_metrics"):
            ...
            if json_metrics["total_saves"] > 0:
                self.execution_data.update(
                    {
                        "json_size_kb": round(
                            json_metrics.get("total_json_size_kb", 0), 2
                        ),
                        "compressed_size_kb": round(
                            json_metrics.get("total_compressed_size_kb", 0), 2
                        ),
                        "compression_ratio": round(
                            json_metrics.get("avg_compression_ratio", 0), 2
                        ),
                    }
                )
    ...
    def finish_execution(self):
        ...
        duration = (
            self.execution_data["ended_at"] - self.execution_data["started_at"]
        )
        self.execution_data["duration_seconds"] = round(
            duration.total_seconds(), 2
        )

        print(
            f"⏱️ Execução finalizada em {self.execution_data['duration_seconds']}s"
        )
    ...
    def save_to_database(self) -> bool:
        ...
        try:
            with get_db_connection() as conn:
                ...
                conn.commit()
                print(
                    f"💾 Log salvo no banco - ID: {self.execution_id[:8]}..."
                )
                return True
    ...
    def print_execution_summary(self):
        ...
        if data["start_date"] and data["end_date"]:
            if data["start_date"] == data["end_date"]:
                ...
                print(
                    f"📆 Período: {data['start_date']} até {data['end_date']}"
                )
    ...
    for exec_data in executions:
        ...
        if exec_data["start_date"] and exec_data["end_date"]:
            if exec_data["start_date"] == exec_data["end_date"]:
                ...
                period = (
                    f" ({exec_data['start_date']} até {exec_data['end_date']})"
                )

        json_indicator = " 📦" if exec_data["json_storage_enabled"] else ""
        records = (
            f"{exec_data['total_records_processed']:,}"
            if exec_data["total_records_processed"]
            else "0"
        )

        print(
            f"{status_icon} {exec_data['execution_type'].upper()}{period}{json_indicator}"
        )
        print(
            f"   ├── ID: {exec_data['execution_id'][:8]}... | {exec_data['started_at'].strftime('%d/%m/%Y %H:%M')}"
        )
        print(
            f"   ├── Duração: {exec_data['duration_seconds']}s | Registros: {records}"
        )

        if exec_data["error_message"]:
            error_preview = (
                exec_data["error_message"][:50] + "..."
                if len(exec_data["error_message"]) > 50
                else exec_data["error_message"]
            )
