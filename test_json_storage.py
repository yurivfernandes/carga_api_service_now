...
def test_json_compression():
    ...
    if loaded_data:
        ...
        print(
            f"   ├── Total de registros: {loaded_data['metadata']['total_records']}"
        )
        print(
            f"   └── Timestamp: {loaded_data['metadata']['extraction_timestamp']}"
        )
        ...
        for table_name, table_data in loaded_data["data"].items():
            ...
            if table_data["records"]:
                ...
                print(
                    f"   │   └── Exemplo: {list(first_record.keys())[:3]}..."
                )
    else:
...
def test_compression_comparison():
    ...
    small_data = {
        "incident": pl.DataFrame(
            [{"sys_id": "test", "description": "Small test"}]
        )
    }
    ...
    for scenario_name, data in scenarios:
        ...
        print(
            f"   └── KB por registro: {metrics['compressed_size_kb'] / total_records:.3f}"
        )
